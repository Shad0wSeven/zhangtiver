...
class ActivePairScalperStrategy:
    ...
    def __init__(self, params: dict | None = None) -> None:
        params = params or {}
        self.entry_sum_max = float(params.get("entry_sum_max", 0.94))
        self.exit_sum_min = float(params.get("exit_sum_min", 0.98))
        self.stop_sum_max = float(params.get("stop_sum_max", 0.90))
        self.max_spread_sum = float(params.get("max_spread_sum", 0.08))
        self.max_mid_imbalance = float(params.get("max_mid_imbalance", 0.22))

        self.per_leg_usd = float(params.get("per_leg_usd", 36.0))
        self.max_hold_s = int(params.get("max_hold_s", 8))
        self.cooldown_s = int(params.get("cooldown_s", 1))
        self.window_low_s = int(params.get("window_low_s", 5))
        self.window_high_s = int(params.get("window_high_s", 80))

        self.recent_sum_n = int(params.get("recent_sum_n", 8))
        self.entry_z_min = float(params.get("entry_z_min", 0.6))
