...
def _load_base():
    p = Path(__file__).with_name("mean_reversion.py")
...
_mod = _load_base()
Base = getattr(_mod, "TrendPullbackStrategy")


def create_strategy(params: dict | None = None):
    p = {'vol_cap_60s': 9.9, 'flip_rate_cap_60s': 1.0, 'jump_cap': 9.9, 'spread_cap': 0.10}
    if params:
        p.update(params)
    s = Base(params=p)
