...
def _load_base():
    p = Path(__file__).with_name("late_flip_hunter.py")
...
_mod = _load_base()
Base = getattr(_mod, "LateFlipHunterStrategy")


def create_strategy(params: dict | None = None):
    p = {'jump_cap': 9.9, 'spread_cap': 0.10, 'entry_gap_min': 0.0, 'entry_bid_min': 0.0}
    if params:
        p.update(params)
    s = Base(params=p)
