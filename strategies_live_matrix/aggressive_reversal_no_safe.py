...
def _load_base():
    p = Path(__file__).with_name("aggressive_reversal.py")
...
_mod = _load_base()
Base = getattr(_mod, "AggressiveReversalStrategy")


def create_strategy(params: dict | None = None):
    p = {'cooldown_s': 0, 'vol_cap_60s': 9.9, 'jump_cap': 9.9, 'max_spread': 0.10}
    if params:
        p.update(params)
    s = Base(params=p)
