...
def _load_base():
    p = Path(__file__).with_name("aggressive_breakout.py")
...
_mod = _load_base()
Base = getattr(_mod, "AggressiveBreakoutStrategy")


def create_strategy(params: dict | None = None):
    p = {'vol_cap_60s': 9.9, 'jump_cap': 9.9, 'spread_cap': 0.10, 'strength_min': 0.0}
    if params:
        p.update(params)
    s = Base(params=p)
